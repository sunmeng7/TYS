...
from utilss.utils import DivideMax
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.target_pos_emb = AxialPositionalEmbedding(
            dim, axial_shape=(num_targets, image_fmap_size, image_fmap_size))   # dim=768 num_targets=8 image_fmap_size=16
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.fixed_language_model is None:
            ...
            text = torch.where(text == 0, text_range, text) # text token
        ...
        control_emb = torch.cat((control_emb, text_emb), dim=1) # control REL VID
        ...
        if not return_loss:
            return control_emb
        ######################################
        target_orig = None  # orig (4,8,3,128,128)
        if exists(target) and len(target):
            target_orig = target.detach().clone()
            target = self.get_image_tokens(target) # target (4,512)
        ...
        # bsize = 6
        for i in range(batch_size): # 6 batchsize
            emb = target_orig[i].reshape(1 * 8, *target_orig.shape[2:5])[0]
            torchvision.utils.save_image(emb[0],
                                         out_path / f'{i}.png',
                                         nrow=8,
                                         normalize=True,
                                         range=(0, 1))
        similarity = deepfaceCos(out_path)
        loss_dis = 0
        for i in range(batch_size):   # 6 batchsize
            for j in range(batch_size):
                if j > i:
                    if similarity[i][j] > 0.88:
                        loss_dis += similarity[i][j] * 4
                    elif similarity[i][j] < 0.54:
                        loss_dis += similarity[i][j] * 3
                    else:
        ...
        # NOTE: Continuity Estimation Task -- VID
        ...
        if vid and self.num_targets > 1:
            ...
            target_warp_masked = torch.where(mask1, target_warp,
                                             self.image_token_lut['[MASK]'])    # Improved Mask-Predict for Video Generation
