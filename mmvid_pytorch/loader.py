...
class TextVideoDataset(Dataset):
    def __init__(
        ...
        min_len = 8
        nframe_num = 2
        self.frame_num = frame_num  # 原值8--4
        self.frame_step = frame_step    # 原值4--2
        ...
        dataroot = str(path) # data/mmvoxceleb/train/video+txt
        self.root = dataroot
        ...
        if cache is not None and cache.exists():
            ...
            self.texts, self.videos, self.lengths = cache_data[     # lengths-图片文件夹下的图片数
            ...
            for i, video in enumerate(
                ...
                if len(frame_list) > 0:
                    ...
                    text_dict[key] = os.path.join('txt', text)  # 文件夹, 要先删掉xx_local.pkl
                    # text_dict[key] = os.path.join('text', text)
        ...
        # 过滤出长度太短的视频    Filter out videos that are too short
    ...
    def _get_label(self, key):
        label_file = Path(
            ...
            # os.path.join(self.root, self.texts[key].replace('text/', 'label/')))
    ...
    def _get_video(self, index, frame_step=None):
        ...
        if True:
            ...
            visual = self.image_transform(to_tensor(visual))    # embedding
