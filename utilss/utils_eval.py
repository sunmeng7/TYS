...
import utilss.utils_html as utils_html
from utilss.utils import mean_pooling, clip_similarity
...
def evaluate(args,
    ...
    with tf.Graph().as_default():
        ...
        with open(OUTPUT_DIR / 'fvd_score.text', 'w') as f:
            f.write(f"{score}")
        ...
        with open(OUTPUT_DIR / 'prd_score.text', 'w') as f:
...
def evaluate_clip(args,
    ...
    clipper = torch.jit.load("ViT-B-32.pt").cuda().eval()
    ...
    for _ in tqdm(range(TOTAL_NUM // batch_size)):
        ...
        if upper_bound:
            ...
            sample_vc, _, _ = dalle_module.generate_images(
                text,
                visual=None,
                erase_visual=False,
                argmax=args.pnag_argmax,
    ...
    with open(OUTPUT_DIR / f'clip{suffix}_score.text', 'w') as f:
