...
from utilss.utils import mean_pooling
from deepface_emb import deepfaceCos
import pickle
# import utils_html
...
def get_dataset(args, tokenizer):
    ...
    if args.dataset == 'video_text':
        ...
        ds = TextVideoDataset(
            ...
            # image_text_folder=args.imageimage_text_folder,
    ...
    elif args.dataset == 'vox':  # text_and_mask
...
def visualize_train(
    ...
    for j in range(N_SAMPLE):
        if args.fixed_language_model is None:
            ...
            text_repeat = text[j:j + 1].repeat(N_PER_SAMPLE, 1) # 复制为两个tensor
        ...
        if args.visual:
            ...
            # print()
            # samples_img.append(frames[j, :N_FRAME, ...])  # real video sequence  --true image
            samples_img.append(frames_recon)    # --重构
    ...
    # # 这里需要加一个文件夹, 读取txt文件名
    # sample_img = samples_img[1]
    # # sam = sample_img[8]
    # # samples_img = torch.cat(samples_img)
    # # 单张图片
    # os.makedirs(SOLO_OUTPUT / fr'{which_iter}', exist_ok=True)
    # for i in range(N_PER_SAMPLE):
    #     torchvision.utils.save_image(sample_img[N_FRAME * i],
    #                                  SOLO_OUTPUT / fr'{which_iter}' / f'{i}.png',
    #                                  nrow=N_FRAME_,
    #                                  normalize=True,
    #                                  range=(0, 1))


    # solo_file = SOLO_OUTPUT / fr'{which_iter}'
    # similarity = deepfaceCos(solo_file)
    # print(similarity)

    # samples_img = samples_img[1]
    samples_img = torch.cat(samples_img)
    ...
    # LOG_SAMPLE_DIR == logs/test_vox_text/samples
...
def visualize_test(
    ...
    N_SAMPLE = min(args.n_sample, args.batch_size)
    N_PER_SAMPLE = args.n_per_sample
    N_FRAME = args.num_targets  # 这个改不了只能为8
    N_FRAME_ = args.num_targets + args.num_visuals
    N_VISUAL = args.num_visuals
    IMAGE_SIZE = args.image_size
    LOG_SAMPLE_DIR = args.log_sample_dir
    which_cvae = 'vae' if args.use_cvae is None else 'cvae'
    # which_cvae = 'vae' if args.use_cvae is False else 'cvae'
    SOLO_OUTPUT = args.solo_output
    ...
    for j in range(N_SAMPLE):
        ...
        # Sample (with visual)
        face_mode = None
        # frames.shape--4, 8, 3, 128, 128
        # frames[0:1, :8, ...].shape--1, 8, 3, 128, 128
        frames_recon = dalle_module.recon_images(frames[j:j + 1, :N_FRAME,
                                                 ...])

        visual = visuals[j:j + 1, ...].repeat(N_PER_SAMPLE, 1, 1, 1,
                                              1) if args.visual else None
        # if args.visual:     # 加入视觉控制
        #     print()
        # else:
        #     print()
        #     # samples_img.append(frames[j, :N_FRAME, ...])  # real video sequence   应该只是用来对比的
    ...
    #     img_path = solo_file / image
    #     img_path = str(img_path)
    #     embeddings.append(DeepFace.represent(img_path=img_path, enforce_detection=False))
    #
    # # 写
    ...
    # similarity = torch.cosine_similarity(embeddings.unsqueeze(1), embeddings.unsqueeze(0), dim=-1)
    # print(similarity)

    # samples_img = samples_img[1]
    samples_img = torch.cat(samples_img)
    torchvision.utils.save_image(samples_img,  # tensor to image
                                 LOG_SAMPLE_DIR / f'{which_iter}.png',
                                 nrow=N_FRAME_,
                                 normalize=True,
                                 range=(0, 1))
    ...
    # # 单张图片
    for i in range(samples_img.shape[0] // N_FRAME):
        os.makedirs(SOLO_OUTPUT / fr'{which_iter}', exist_ok=True)
        torchvision.utils.save_image(samples_img[N_FRAME * i],
                                     SOLO_OUTPUT / fr'{which_iter}' / f'{i}.png',
                                     nrow=N_FRAME_,
                                     normalize=True,
                                     range=(0, 1))
