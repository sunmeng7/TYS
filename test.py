import os
# os.environ["CUDA_VISIBLE_DEVICES"] = '1'    # 显卡块, 默认0, 这里选1
...
from utilss.utils_train import visualize_test as visualize
from utilss.utils_train import visualize_long
from utilss.utils_eval import evaluate, evaluate_clip
from utilss.utils import exists, set_requires_grad, sample_data, \
...
def main_worker(args):
    ...
    solo_output = Path(args.solo_output)
    ...
    if args.ar:
        ...
        dalle = BERT(vae=vae, cvae=cvae, **dalle_params)    # model
    if args.fp16:
        dalle = dalle.half()
    ...
    print(f'{len(ds)} image-text pairs found for testing')
    ...
    dl_iter = sample_data(dl, data_sampler)     # 迭代器取值
